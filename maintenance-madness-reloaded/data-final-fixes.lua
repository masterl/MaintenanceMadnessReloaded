...
for _, chest in pairs(data.raw.container) do
	if internalSettings.maintenanceUnits[chest.name] then
		...
		local hiddenEnergyInterface = table.deepcopy(data.raw["electric-energy-interface"]["hidden-electric-energy-interface"])
		...
		hiddenEnergyInterface.localised_description = data.raw["container"][chest.name].localised_description
		...
		hiddenEnergyInterface.flags = {"placeable-neutral", "player-creation", "placeable-off-grid", "not-on-map", "not-blueprintable", "not-deconstructable", "hide-alt-info"}
		local energySource = {
			type = "electric",
			usage_priority = "secondary-input",
			buffer_capacity = internalSettings.maintenanceUnits[chest.name].energyBuffer,
			input_flow_limit = internalSettings.maintenanceUnits[chest.name].energyUsage,
		...
		hiddenEnergyInterface.energy_usage = internalSettings.maintenanceUnits[chest.name].energyUsage
		...
		local placementEntity = {
			...
			supply_area_distance = internalSettings.maintenanceUnits[chest.name].radius,
		...
		local placementEntityRecipe = table.deepcopy(data.raw.recipe[chest.name])
		...
		-- Add this recipe to the "repair-and-maintenance" tech
			...
			table.insert(data.raw.technology["mm-repair-and-maintenance"].effects, recipeEffect)
		end
...
for prototypeGroup, _ in pairs(internalSettings.entityTypesWithMROenabled) do
	...
	for __, entityPrototype in pairs(data.raw[prototypeGroup]) do
		...
		if string.sub(protName, 1, 10) == "mm-faulty-" or prototypeGroup == "beacon" and string.find(protName, "-placement%-entity") ~= nil then
			goto continue
		...
		if data.raw.recipe[protName] then
			...
			log("Missing recipe for: "..protName..". Maintenance for this entity will be disabled.")
			goto continue
		end
		if data.raw.item[protName] then
			...
			log("Missing item for: "..protName..". Maintenance for this entity will be disabled.")
			goto continue
		end
		...
		if item.icons then -- Multi layered icons
			...
			for _, itemLayer in pairs(itemIcon) do
				...
				itemLayer.scale = (32 / (itemLayer.icon_size or item.icon_size or 64)) * 0.9 -- bigger icon sizes will be resized to fit into 32x32
			end
			table.insert(itemIcon, 1, {icon = "__base__/graphics/icons/remnants.png", tint = {a = 1, r = 0.8, g = 0.4, b = 0.3}, icon_size = 64, icon_mipmaps = 4})
		else
			local itemIconScale = (32 / item.icon_size) * 0.9 -- bigger icon sizes will be resized to fit into 32x32
			itemIcon = {
				{icon = "__base__/graphics/icons/remnants.png", tint = {a = 1, r = 0.8, g = 0.4, b = 0.3}, icon_size = 64, icon_mipmaps = 4},
				{icon = item.icon, scale = itemIconScale, tint = {a = 1, r = 0.8, g = 0.4, b = 0.3}, icon_size = item.icon_size, icon_mipmaps = item.icon_mipmaps}
			}
		end
		...
		-- Basic info
			...
			localised_name = {"mm-item-name-scrap", {"entity-name."..protName}},
			--icon = item.icon,
			icons = itemIcon,
		...
		table.insert(recyclingIcon, {icon = "__maintenance-madness-reloaded__/graphics/icons/recycle-32px.png", scale = 0.7, icon_size = 32})

		local recyclingRecipe = {
			type = "recipe",
			name = "mm-recycle-"..protName,
			localised_name = {"mm-recipe-name-recyling", {"entity-name."..protName}},
		...
		local recyclingRecipeSpecs = {
			...
			energy_required = math.max(timeNeeded * recycleTimeMultiplier, internalSettings.recycleTimeMinimum),
			allow_decomposition = false,
		...
		table.insert(reconditionIcon, {icon = "__base__/graphics/icons/repair-pack.png", scale = (32 / 64) * 0.7, icon_size = 64, icon_mipmaps = 4})

		local reconditionRecipe = {
			type = "recipe",
			name = "mm-recondition-"..protName,
			localised_name = {"mm-recipe-name-recondition", {"entity-name."..protName}},
		...
		local reconditionRecipeSpecs = {
			...
			ingredients = {{"mm-scrapped-"..protName, 1}, {"mm-toolbox", 1}},
			energy_required = math.max(timeNeeded * reconditionTimeMultiplier, internalSettings.reconditionTimeMinimum),
			allow_decomposition = false,
			allow_as_intermediate = false,
			allow_intermediates = false,
			always_show_products = true,
		...
		-- If this is a recipe with "normal" and/or "expensive" cost, create normal/expensive recipe cost for the recycling/recondition items, too
			if recipe.normal then
				...
				reconditionRecipe.normal = table.deepcopy(reconditionRecipeSpecs)
				for _, ingredient in pairs(recipe.normal.ingredients) do
					...
					if ingredient.type ~= "fluid" and amount >= 2 then
						...
						table.insert(reconditionRecipe.normal.ingredients, {
							type = "item",
							name = ingredient.name or ingredient[1],
							amount = math.floor(amount * internalSettings.reconditionSparePartFactor),
						})
					end
				end
			else
			...
			if recipe.expensive then
				recyclingRecipe.expensive = table.deepcopy(recyclingRecipeSpecs)
				reconditionRecipe.expensive = table.deepcopy(reconditionRecipeSpecs)
			...
			mm_util.appendTable(recyclingRecipe, table.deepcopy(recyclingRecipeSpecs))
			mm_util.appendTable(reconditionRecipe, table.deepcopy(reconditionRecipeSpecs))
		...
		-- This chest is required as a target for item request proxies
			...
			flags = {"placeable-neutral", "player-creation", "placeable-off-grid", "not-on-map", "not-blueprintable", "not-deconstructable", "not-upgradable", "hide-alt-info", "no-automated-item-removal", "no-automated-item-insertion"},
			--minable = false,
		...
		local dimensions = mm_util.getBoundingBoxDimensions((requesterContainer.selection_box or requesterContainer.collision_box))
		...
		if dimensions.width ~= dimensions.height then
			...
			requesterContainerRotated.collision_box = mm_util.rotateBoundingBox(requesterContainer.collision_box)
			requesterContainerRotated.selection_box = mm_util.rotateBoundingBox(requesterContainer.selection_box)
		end
		...
		-- Add this recipe to the "recondition" tech
			...
			table.insert(data.raw.technology["mm-recondition"].effects, recipeEffect)
		end
		...
		-- If a solar panel or an accumulator breaks down, it has to be replaced by a "faulty" one by script.
			...
			if item.icons then -- Multi layered icons
				...
				table.insert(faultyEntityIcon, {icon = "__maintenance-madness-reloaded__/graphics/icons/machine-malfunction-small.png", scale = maxIconSize / 32, icon_size = 32})				
			else
				faultyEntityIcon = {
					{icon = item.icon, scale = maxIconSize / (item.icon_size or 32), icon_size = item.icon_size, icon_mipmaps = item.icon_mipmaps},
					{icon = "__maintenance-madness-reloaded__/graphics/icons/machine-malfunction-small.png", scale = maxIconSize / 32, icon_size = 32}
				}
			end

			local faultyEntity = table.deepcopy(entityPrototype)
			faultyEntity.name = "mm-faulty-"..protName
			faultyEntity.localised_name = {"mm-faulty-entity-name", {"entity-name."..protName}}
			...
			if entityPrototype.localised_desctription ~= nil then
				faultyEntity.localised_desctription = {"entity-description."..protName}
			end
...
local proxies = {["mm-maintenance-request-proxy"] = {"mm-generic-manual-spareparts"},
...
for proxyName, additionalDescription in pairs(proxies) do
	...
	data.raw["item-request-proxy"][proxyName].localised_description = {"mm-style-label-entity-description", locale, additionalDescription}
end
