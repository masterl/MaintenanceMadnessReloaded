...
local function buildFooter(parent, name, data)
    ...
    if data.backButton then
        local buttonName = prefix .. (data.backButtonName or "footer-closeButton")
        footer_flow.add{
            type = 'button',
            name = buttonName,
            style = 'back_button',
    ...
    if data.confirmButton then
        ...
        global.userInterface[data.player_index].buttonReference[(prefix .. "confirmButton")] = button
...
local function addTabAndPanel(gui, name, caption)
    ...
    local tab = tabContainer.add{type="tab", name="maintenanceMadness-tab-"..name, caption=caption}
    local content = tabContainer.add{type="flow", name="maintenanceMadness-tab-"..name.."-frame", direction="vertical"}
...
local function getEntityCount(entityName, player_index)
    ...
    if entityType == "solar-panel" or entityType == "accumulator" then
        ...
        count = get_entity_count(entityName) + get_entity_count("mm-faulty-"..entityName)
    else
...
local function getSortedEntityList(player_index, maintenanceControlData)
    ...
    for entityName, controlData in pairs(maintenanceControlData) do
        sortedEntityList[#sortedEntityList+1] = {name = entityName, count = getEntityCount(entityName, player_index)}
    end
    table.sort(sortedEntityList, function(a,b) return a.count>b.count end)
...
local function createNoMachinesLabel(parent)
    ...
    local sb = flow.add{type="sprite-button", sprite = "utility/warning_white", style = "transparent_slot"}
    flow.add{type="label", style = "label_with_left_padding", caption = {"gui.mm-no-machines"}}
end
...
local function populateItemPolicyTable(parent, player_index, policyType)
    ...
    local buttonReference = global.userInterface[player_index].buttonReference or {}
    if global.temporaryMaintenanceControl and global.temporaryMaintenanceControl[player_index] then
        local control = global.temporaryMaintenanceControl[player_index]["byEntity"]
        ...
        for _, element in pairs(order) do
            if element.count > 0 then
                ...
                local flow1 = parent.add{type="flow", direction="horizontal"}
                local sb = flow1.add{type="sprite-button", sprite = "item/"..element.name, style = "transparent_slot", number = element.count}
                flow1.add{type="label", style = "label_with_left_padding", caption = {"entity-name."..element.name}}
                ...
                if policyType == "maintenance" or policyType == "repair" then
                    ...
                    local itemCount = mmUtil.getLength(control[element.name][policyType])
                    local repairItemData, repairItemCount
                    if policyType == "maintenance" then
                        itemData = global.entitiesWithMROenabled[element.name][policyType][const.maintenanceLevel]
                        itemSpritePathG = "maintenance-needed-grey-icon"
                        itemSpritePath = "maintenance-needed-icon"
                    elseif policyType == "repair" then
                        itemData = global.entitiesWithMROenabled[element.name][policyType][const.maintenanceLevel].secondary
                        repairItemData = global.entitiesWithMROenabled[element.name][policyType][const.maintenanceLevel].primary
                    ...
                    local effectLabelNameComplete = "maintenanceMadness-itemPolicyEffectLabel-"..policyType.."-"..element.name
                    local buttonName = "maintenanceMadness-itemPolicyButton-"..policyType.."-"..element.name.."-"
                    ...
                    if itemCount > 5 then
                        manyItems = true
                        flow2 = parent.add{type="flow", direction="vertical"}
                        flow2.style.vertical_spacing = 4
                        flow2a = flow2.add{type="flow", direction="horizontal"}
                        flow2b = flow2.add{type="flow", direction="horizontal"}
                    else
                        flow2 = parent.add{type="flow", direction="horizontal"}
                    end
                    if policyType == "repair" then
                        for repairItemName, _ in pairs(repairItemData) do
                            local buttonNameComplete = buttonName..repairItemName
                            local repairItemButton
                            itemNo = itemNo + 1

                            if manyItems then
                                if itemNo % 2 == 1 then
                                    repairItemButton = flow2a.add{type="sprite-button", name = buttonNameComplete, sprite = "item/"..repairItemName, ignored_by_interaction = true, style = "blue_slot", tooltip = getLocalisedName(repairItemName)}
                                else
                                    repairItemButton = flow2b.add{type="sprite-button", name = buttonNameComplete, sprite = "item/"..repairItemName, ignored_by_interaction = true, style = "blue_slot", tooltip = getLocalisedName(repairItemName)}
                                end
                            else
                                repairItemButton = flow2.add{type="sprite-button", name = buttonNameComplete, sprite = "item/"..repairItemName, ignored_by_interaction = true, style = "blue_slot", tooltip = getLocalisedName(repairItemName)}
                            end
                        end
                    end

                    for itemName, use in pairs(control[element.name][policyType]) do
                        ...
                        if manyItems then
                            if itemNo % 2 == 1 then
                                if use.enabled == true then
                                    flow2a.add{type="sprite-button", name = buttonNameComplete, sprite = "item/"..itemName, style = "green_slot", tooltip = getLocalisedName(itemName)}
                                    effectPercent = effectPercent + itemData[itemName].expectedEffect
                                else
                                    flow2a.add{type="sprite-button", name = buttonNameComplete, sprite = "item/"..itemName, style = "red_slot", tooltip = getLocalisedName(itemName)}
                                end
                            else
                                if use.enabled == true then
                                    flow2b.add{type="sprite-button", name = buttonNameComplete, sprite = "item/"..itemName, style = "green_slot", tooltip = getLocalisedName(itemName)}
                                    effectPercent = effectPercent + itemData[itemName].expectedEffect
                                else
                                    flow2b.add{type="sprite-button", name = buttonNameComplete, sprite = "item/"..itemName, style = "red_slot", tooltip = getLocalisedName(itemName)}
                                end
                            end
                        else
                            if use.enabled == true then
                                flow2.add{type="sprite-button", name = buttonNameComplete, sprite = "item/"..itemName, style = "green_slot", tooltip = getLocalisedName(itemName)}
                                effectPercent = effectPercent + itemData[itemName].expectedEffect
                            else
                                flow2.add{type="sprite-button", name = buttonNameComplete, sprite = "item/"..itemName, style = "red_slot", tooltip = getLocalisedName(itemName)}
                            end
                        end
                        global.userInterface[player_index].buttonReference[buttonNameComplete] = {policyType = policyType, entity = element.name, item = itemName, effectLabelName = effectLabelNameComplete}
                    end
                    if manyItems and itemNo % 2 == 1 then
                        local fillerButton = flow2b.add{type="sprite-button", style = "transparent_slot"}
                    ...
                    local flow3 = parent.add{type="flow", direction="horizontal"}
                    flow3.style.vertical_align = "center"
                    local effectIcon = flow3.add{type="sprite-button", name = effectLabelNameComplete.."-icon", sprite = spritePath,  style = "transparent_slot"}
                    local effectLabel = flow3.add{type="label", name = effectLabelNameComplete, style = "count_label", caption = format_percentage(effectPercent, 0.001)}
                    global.userInterface[player_index].labelReference[effectLabelNameComplete] = effectLabel
                    global.userInterface[player_index].iconReference[effectLabelNameComplete.."-icon"] = effectIcon

                    local warningArrow = flow3.add{type="sprite-button", sprite = "utility/expand",  style = "transparent_slot"}
                    global.userInterface[player_index].iconReference[effectLabelNameComplete.."-warning-arrow"] = warningArrow
                    local warningIcon = flow3.add{type="sprite-button", style = "transparent_slot"}
                    global.userInterface[player_index].iconReference[effectLabelNameComplete.."-warning-icon"] = warningIcon
                    local warningLabel = flow3.add{type="label", name = effectLabelNameComplete.."-warning", style = "count_label"}
                    warningLabel.style.font_color = const.textColors.negative
                    global.userInterface[player_index].labelReference[effectLabelNameComplete.."-warning"] = warningLabel


                    if (mceil(100000 * effectPercent) / 100000) < 1 then
                        if policyType == "maintenance" then
                            local warningPercent = 1/(1/(((1-effectPercent)*(const.maxMissedMaintenanceMalus/const.baseAgeing)+1)-1))
                            warningLabel.caption = "+"..format_percentage(warningPercent, 0.01)
                            warningIcon.sprite = "item/mm-scrapped-"..element.name --"machine-malfunction-icon"
                        elseif policyType == "repair" then
                            local warningPercent = (1-effectPercent)/(1/((1/const.baseRepairEffectivity)-1))
                            warningLabel.caption = "+"..format_percentage(warningPercent, 0.01)
...
local function calculateFailureRate(machineAge)
    if machineAge > 0 then
        ...
        local failureProb = const.repairProbabilityModifier * (avgMachineAge / const.maxAge)
        local failureTime = const.MTTR + ((avgMachineAge / const.maxAge) * const.repairTimeModifier * const.MTTR)
        local defaultCycleTime = const.MTBM + (1 - failureProb) * const.MTTM + failureProb * failureTime
...
local function populateReplacementPolicyTable(parent, player_index)
    ...
    if global.temporaryMaintenanceControl and global.temporaryMaintenanceControl[player_index] then
        ...
        for _, element in pairs(order) do
            if element.count > 0 then
                local sliderNameComplete = "maintenanceMadness-replacmentPolicySlider-"..element.name
                local forcedSliderNameComplete = "maintenanceMadness-forcedReplacmentPolicySlider-"..element.name
                local effectLabelNameComplete = "maintenanceMadness-replacementPolicyEffectLabel-"..element.name
                ...
                if valueStart < valueLimit then
                    effectLabelCaption = format_percentage(valueStart / 100, 0.01).." … "..format_percentage(valueLimit / 100, 0.01)
                elseif valueStart == valueLimit then
                    effectLabelCaption = "= "..format_percentage(valueStart / 100, 0.01)
                ...
                flow1.style.vertical_align = "center"
                --local effectPercent = 0
                local flow2 = parent.add{type="flow", direction="vertical"}
                --flow2.style.vertical_align = "center"
                flow2.style.horizontal_align = "right"
                flow2.style.minimal_height = 36
                local slider1 = flow2.add{type="slider", name = sliderNameComplete, style = "notched_slider", minimum_value = 1, maximum_value = 20, value = replacementAgeSetSliderValue(control[element.name].replacement.start), value_step = 1, discrete_slider = true, discrete_values = true}
                global.userInterface[player_index].buttonReference[sliderNameComplete] = {element = slider1, policyType = policyType, entity = element.name, effectLabelName = effectLabelNameComplete, sibling = forcedSliderNameComplete}
                ...
                local slider2 = flow2.add{type="slider", name = forcedSliderNameComplete, style = "notched_slider", minimum_value = 1, maximum_value = 20, value = replacementAgeSetSliderValue(control[element.name].replacement.limit), value_step = 1, discrete_slider = true, discrete_values = true}
                global.userInterface[player_index].buttonReference[forcedSliderNameComplete] = {element = slider2, policyType = policyType, entity = element.name, effectLabelName = effectLabelNameComplete, sibling = sliderNameComplete}
                ...
                local flow3 = parent.add{type="flow", direction="horizontal"}
                flow3.style.vertical_align = "center"
                local flow3a = flow3.add{type="flow", direction="vertical"}
                flow3a.style.vertical_align = "center"
                flow3a.style.horizontal_align = "center"
                local flow3aa = flow3a.add{type="flow", direction="horizontal"}
                flow3aa.style.vertical_align = "center"
                flow3aa.style.horizontal_align = "center"
                local flow3ab = flow3a.add{type="flow", direction="horizontal"}
                flow3ab.style.vertical_align = "center"
                flow3ab.style.horizontal_align = "center"
                local noReplacementLabel = flow3aa.add{type="label", name = effectLabelNameComplete.."-no-replacement", style = "bold_label", caption = {"gui.mm-no-replacement"}}
                noReplacementLabel.visible = false
                global.userInterface[player_index].labelReference[effectLabelNameComplete.."-no-replacement"] = noReplacementLabel
                local effectIcon = flow3aa.add{type="sprite-button", name = effectLabelNameComplete.."-icon", sprite = "replacement-request-icon",  style = "transparent_slot"}
                local effectLabel = flow3ab.add{type="label", name = effectLabelNameComplete, style = "count_label", caption = effectLabelCaption}
                global.userInterface[player_index].labelReference[effectLabelNameComplete] = effectLabel
                ...
                local warningArrow = flow3.add{type="sprite-button", sprite = "utility/expand",  style = "transparent_slot"}
                global.userInterface[player_index].iconReference[effectLabelNameComplete.."-warning-arrow"] = warningArrow
                local flow3b = flow3.add{type="flow", direction="vertical"}
                local flow3c = flow3.add{type="flow", direction="vertical"}

                local warningIcon_1 = flow3b.add{type="sprite-button", style = "transparent_slot"}
                global.userInterface[player_index].iconReference[effectLabelNameComplete.."-warning-icon-1"] = warningIcon_1
                local warningIcon_2 = flow3b.add{type="sprite-button", style = "transparent_slot"}
                global.userInterface[player_index].iconReference[effectLabelNameComplete.."-warning-icon-2"] = warningIcon_2
                local warningLabel_1 = flow3c.add{type="label", name = effectLabelNameComplete.."-warning-1", style = "count_label"}
                warningLabel_1.style.height = 36
                warningLabel_1.style.vertical_align = "center"
                global.userInterface[player_index].labelReference[effectLabelNameComplete.."-warning-1"] = warningLabel_1
                local warningLabel_2 = flow3c.add{type="label", name = effectLabelNameComplete.."-warning-2", style = "count_label"}
                warningLabel_2.style.height = 36
                warningLabel_2.style.vertical_align = "center"
                global.userInterface[player_index].labelReference[effectLabelNameComplete.."-warning-2"] = warningLabel_2
                local warningIcon_3 = flow3.add{type="sprite-button", style = "transparent_slot", tooltip = {"gui.mm-tooltip-warning-operation-age"}}
                global.userInterface[player_index].iconReference[effectLabelNameComplete.."-warning-icon-3"] = warningIcon_3
                ...
                if control[element.name].replacement.start > 0 then
                    local machineFailureRateIndex = calculateFailureRate(const.replacementAge) -- falure rate at default replacement value
                    local thisMachineFailureRate = calculateFailureRate(control[element.name].replacement.start)
                    machineFailureRate = (thisMachineFailureRate / machineFailureRateIndex) - 1

                    local machineReplacementCostIndex = 1 / (const.replacementAge / const.maxAge)
                    local thisMachineReplacementCost = 1 / (control[element.name].replacement.start / const.maxAge)
                    machineReplacementCost = (thisMachineReplacementCost / machineReplacementCostIndex) - 1
                else
                    ...
                    warningIcon_3.tooltip = {"gui.mm-tooltip-danger-operation-age"}
                end
                if control[element.name].replacement.limit == 0 then
                    slider2.style = "maintenanceMadness_red_notched_slider"
                end

                if control[element.name].replacement.start == 0 or control[element.name].replacement.start > 100 then
                    warningIcon_3.visible = true
                else
                    warningIcon_3.visible = false
                end
                if control[element.name].replacement.start ~= const.replacementAge then
                    ...
                    warningLabel_1.caption = warningLabel_1.caption..format_percentage(machineReplacementCost,0.01)
                    if tonumber(machineFailureRate) ~= nil then
                        if machineFailureRate > 0 then
                            ...
                            warningLabel_2.caption = ""
                            warningLabel_2.style.font_color = const.textColors.positive
                        end
                        warningLabel_2.caption = warningLabel_2.caption..format_percentage(machineFailureRate,0.01)
                    else
...
function confirmNewMaintenanceControlSettings(player_index, reset)
            ...
            for entityName, controls in pairs(changedControls.byEntity) do
                if controls.maintenance then
                    for itemName, setting in pairs(controls.maintenance) do
                        updatedControls.byEntity[entityName].maintenance[itemName].enabled = setting.enabled
                    end
                end
                if controls.repair then
                    for itemName, setting in pairs(controls.repair) do
                        updatedControls.byEntity[entityName].repair[itemName].enabled = setting.enabled
                    end
                end
                if controls.replacement then
                    for setting, value in pairs(controls.replacement) do
                        updatedControls.byEntity[entityName].replacement[setting] = value
                    end
                end
            end
        end
    ...
    if game.is_multiplayer() then
        game.forces[forceID].print({"mm-notification-maintenance-settings-changed-MP", game.players[player_index].name})
    end
end

function countChangedSettings(forceID)
    local numOptions = 0
    if global.maintenanceControl[forceID] and global.maintenanceControl[forceID].byEntity then
        for _, entity in pairs(global.maintenanceControl[forceID].byEntity) do
            for _, item in pairs(entity.maintenance) do
                if item.enabled == false then
...
function toggleConfirmResetDialog(player_index)
    ...
    local root = ui.root
    root.gui2 = player.gui.screen.add{type = 'frame', name = 'maintenanceMadness-notification-window', direction = 'vertical'}
    ...
    flow1.style.vertical_align = "center"
    flow1.add{type = "label", style = "label_with_left_padding", caption = {"reset-to-defaults", numOptions}}
    gui2.style.minimal_height = 125
    gui2.style.minimal_width = 350
    buildFooter(gui2, "maintenanceMadness-window-footer", {backButton = true, backButtonName = "quitResetConfirmationDialog", confirmResetButton = true, draggable = true, player_index = player_index})
...
function toggleConfirmationDialog(player_index)
    ...
    flow1.style.vertically_stretchable = true
    flow1.style.vertical_align = "center"
    flow1.add{type = "label", style = "label_with_left_padding", caption = {"unconfirmed-changes", global.changedControlSettings[player_index].total}}
    gui2.style.minimal_height = 125
    gui2.style.minimal_width = 350
    buildFooter(gui2, "maintenanceMadness-window-footer", {backButton = true, backButtonName = "quitConfirmationDialog", discardChangesButton = true, draggable = true, player_index = player_index})
...
function updateLegendInformation(player_index, tab_index, hide)
    ...
    interactionLegendCaption.maintenance = {["red"] = {"gui.mm-legend-maintenance-red-button"},
                                            ["green"] = {"gui.mm-legend-maintenance-green-button"}}
    interactionLegendCaption.repair = {    ["red"]    = {"gui.mm-legend-repair-red-button"},
    ...
    legendCaptions.maintenance = {    {"gui.mm-legend-maintenance-detail1", "[img=maintenance-needed-grey-icon] "},
                                    {"gui.mm-legend-maintenance-detail2", "[img=maintenance-needed-icon] "},
                                    {"gui.mm-legend-maintenance-detail3", "[item=mm-scrapped-assembling-machine-1] "}}
    legendCaptions.repair = {    {"gui.mm-legend-repair-detail1", "[img=repair-in-progress-gray-icon] "},
                                {"gui.mm-legend-repair-detail2", "[img=repair-in-progress-icon] "},
                                {"gui.mm-legend-repair-detail3", "[img=utility/clock] "}}
    legendCaptions.replacement = {    {"gui.mm-legend-replacement-detail1", "[img=replacement-request-icon] "},
                                    {"gui.mm-legend-replacement-detail2", "[item=mm-scrapped-assembling-machine-1] "},
...
function mmGUI.toggleMasterPanel(player_index, update)
    ...
    local player = game.players[player_index]
    local forceID = game.players[player_index].force.index
    global.temporaryMaintenanceControl[player_index] = global.temporaryMaintenanceControl[player_index] or global.maintenanceControl[forceID]
    ...
    local container = gui.add{type = "frame", name = "maintenanceMadness-window-container", direction = 'vertical', style = "inside_deep_frame"}
    container.style.horizontally_stretchable = true
    container.style.maximal_height = (player.display_resolution.height - (240 * player.display_scale)) / player.display_scale

    local subheader = container.add{type="frame", name="maintenanceMadness-window-subheader-frame", direction="horizontal", style = "subheader_frame"}
    subheader.style.horizontally_stretchable = true
    local subheaderFlow = subheader.add{type="flow", direction="horizontal"}
    ...
    local resetButton = subheaderFlow.add{type="sprite-button", name = "maintenanceMadness-resetButton",  sprite = "utility/reset", style = "tool_button_red"}
    if global.maintenanceControl[forceID].numOptions > 0 then
        resetButton.tooltip = {"reset-to-defaults", global.maintenanceControl[forceID].numOptions}
    else
        resetButton.tooltip = {"reset-to-defaults-disabled"}
        resetButton.enabled = false
    end
    if const.lowTechModifierEnabled then
        local lowTechBarFrame = container.add{type="flow", direction = "horizontal"}
        lowTechBarFrame.style.left_margin = 13
        lowTechBarFrame.style.right_margin = 10
        lowTechBarFrame.style.top_margin = 6
        local tooltip = {"gui.mm-window-tab-statusbar-lowtech-bonus-tooltip",
        {"gui.mm-window-tab-statusbar-lowtech-bonus-tooltip-description"},
        {"gui.mm-window-tab-statusbar-lowtech-bonus-tooltip-1", {"technology-name."..const.lowTechModifierTarget}},
        {"gui.mm-window-tab-statusbar-lowtech-bonus-tooltip-2", mmUtil.getLength(global.lowTechModifier[forceID].techs)},
        {"gui.mm-window-tab-statusbar-lowtech-bonus-tooltip-3", global.lowTechModifier[forceID].researchedUnits, global.lowTechModifier[forceID].totalUnits}}
        local lowTechBarLabel = lowTechBarFrame.add{type="label", style = "description_property_name_label", caption = {"gui.mm-window-tab-statusbar-lowtech-bonus"}, tooltip = tooltip}
        local lowTechBarBar = lowTechBarFrame.add{type = "progressbar", value = global.lowTechModifier[forceID].factor, tooltip = tooltip}
        ...
        local lowTechBarPercentage = lowTechBarFrame.add{type = "label", caption = format_percentage(global.lowTechModifier[forceID].factor, 0.001), tooltip = tooltip}
    end
    root.tabContainer = container.add{type="tabbed-pane", name = "maintenanceMadness-tabContainer"}
    root.tabContainer.style.top_padding = 12

    for name, data in pairs(
            {
                ["maintenance"] = {caption={"gui.mm-window-tab-title-maintenance"}, column_count=3},
                ["repair"] = {caption={"gui.mm-window-tab-title-repair"}, column_count=3},
                ["overhaul"] = {caption={"gui.mm-window-tab-title-overhaul"}, column_count=3},
                --["statistics"] = {caption="Statistics", column_count=3},
            }
    ) do
        root.tabpanes[name] = addTabAndPanel(root, name, data.caption)
        local tableHeaderFrame = root.tabpanes[name].add{type="frame", direction="horizontal", style="subpanel_frame"}
        tableHeaderFrame.style.left_margin = 1

        local tableHeader = tableHeaderFrame.add{type="table", column_count=data.column_count, style = "maintenanceMadness_item_header_table"}
        tableHeader.style.horizontally_stretchable = true

        local scrollpane = root.tabpanes[name].add{type="scroll-pane", style = "tab_scroll_pane", vertical_scroll_policy = "always", horizontal_scroll_policy = "auto"}
        ...
        local table = scrollpane.add{type="table", column_count=data.column_count, style = "maintenanceMadness_item_table"}

        if name == "maintenance" then
            tableHeader.add{type="label", style = "orange_label", caption = {"gui.mm-window-tab-maintenance-header-name"}}
            tableHeader.add{type="label", style = "orange_label", caption = {"gui.mm-window-tab-maintenance-header-setting"}}
            tableHeader.add{type="label", style = "orange_label", caption = {"gui.mm-window-tab-maintenance-header-effect"}}
        ...
        if name == "repair" then
            tableHeader.add{type="label", style = "orange_label", caption = {"gui.mm-window-tab-repair-header-name"}}
            tableHeader.add{type="label", style = "orange_label", caption = {"gui.mm-window-tab-repair-header-setting"}}
            tableHeader.add{type="label", style = "orange_label", caption = {"gui.mm-window-tab-repair-header-effect"}}
            if not populateItemPolicyTable(table, player_index, "repair") then
                table.destroy()
                createNoMachinesLabel(scrollpane)
            end
        elseif name == "overhaul" then
            tableHeader.add{type="label", style = "orange_label", caption = {"gui.mm-window-tab-overhaul-header-name"}}
            tableHeader.add{type="label", style = "orange_label", caption = {"gui.mm-window-tab-overhaul-header-setting"}}
            tableHeader.add{type="label", style = "orange_label", caption = {"gui.mm-window-tab-overhaul-header-effect"}}
    ...
    local subfooter = container.add{type = "frame", name = "maintenanceMadness-subfooter", direction = "horizontal", style = "subfooter_frame"}
    subfooter.style.horizontally_stretchable = true
    subfooter.style.left_padding = 8
    local subfooter_flow = subfooter.add {type = "flow", name = "maintenanceMadness-subfooter-flow", direction = "vertical"}
    subfooter_flow.style.minimal_height = 24
    subfooter_flow.style.top_padding = 1
    local subfooter_headerFlow = subfooter_flow.add {type = "flow", name = "maintenanceMadness-subfooter-headerFlow", direction = "horizontal"}
    ...
    legend.informationLabel = subfooter_headerFlow.add{type = "label", name = "maintenanceMadness-subfooter-informationLabel", style = "caption_label"}
    legend.informationLabel.caption = "Click on any item to toggle its use for maintenance or repair operations"
    legend.informationLabel.style.bottom_padding = 2
    local filler = subfooter_headerFlow.add{type = "flow", direction = "horizontal"}
    filler.style.horizontally_stretchable = true
    legend.showLegendLabel = subfooter_headerFlow.add{type="label", style = "info_label"}
    legend.showLegendLabel.style.bottom_padding = 2
    legend.showLegendLabel.style.right_margin = 3
    legend.showLegendButton = subfooter_headerFlow.add{type="sprite-button", name = "maintenanceMadness-toggleLegendButton",  sprite = "utility/expand_dark", style = "tool_button"}
    legend.showLegendButton.style.padding = -2
    legend.contentFlow1 = subfooter_flow.add{type = "flow", direction = "horizontal"}
    legend.contentFlow1.style.natural_height = 24
    --subfooter_flow.style.vertical_align = "center"

    legend.icon1 = legend.contentFlow1.add{type="sprite-button", style = "blue_slot"}
    ...
    legend.label1 = legend.contentFlow1.add{type = "label", name = "maintenanceMadness-subfooter-legendLabel1", style = "info_label"}
    legend.label1.style.right_margin = 14
    legend.label1.style.left_margin = 3

    legend.icon2 = legend.contentFlow1.add{type="sprite-button", style = "green_slot"}
    ...
    legend.label2 = legend.contentFlow1.add{type = "label", name = "maintenanceMadness-subfooter-legendLabel2", style = "info_label"}
    legend.label2.style.right_margin = 14
    legend.label2.style.left_margin = 3

    legend.icon3 = legend.contentFlow1.add{type="sprite-button", style = "red_slot"}
    ...
    legend.label3 = legend.contentFlow1.add{type = "label", name = "maintenanceMadness-subfooter-legendLabel3", style = "info_label"}
    legend.label3.style.right_margin = 14
    legend.label3.style.left_margin = 3
    legend.label4 = legend.contentFlow1.add{type = "label", name = "maintenanceMadness-subfooter-legendLabel4", style = "info_label"}
    legend.label4.style.right_margin = 14

    legend.contentFlow2 = subfooter_flow.add{type = "flow", direction = "horizontal"}
    legend.contentFlow2.style.minimal_height = 24
    subfooter_flow.style.vertical_align = "center"
    legend.label5 = legend.contentFlow2.add{type = "label", name = "maintenanceMadness-subfooter-legendLabel5", style = "info_label"}
    legend.label5.style.right_margin = 14
    legend.label6 = legend.contentFlow2.add{type = "label", name = "maintenanceMadness-subfooter-legendLabel6", style = "info_label"}
    legend.label6.style.right_margin = 14
    legend.label7 = legend.contentFlow2.add{type = "label", name = "maintenanceMadness-subfooter-legendLabel7", style = "info_label"}
    legend.label7.style.right_margin = 14

    buildFooter(gui, "maintenanceMadness-window-footer", {backButton = true, confirmButton = true, draggable = true, player_index = player_index})

    root.tabContainer.selected_tab_index = ui.selectedTab or 1
    updateLegendInformation(player_index, root.tabContainer.selected_tab_index, ui.hideLegend)
end

function mmGUI.updateReplacementPolicySlider(element, player_index)
    local buttonReference = global.userInterface[player_index].buttonReference[element.name]
    local confirmButton = global.userInterface[player_index].buttonReference["maintenanceMadness-window-footer-confirmButton"]
    if buttonReference then
        local sliderValue = replacementAgeGetSliderValue(element.slider_value)
        local sibling = global.userInterface[player_index].buttonReference[buttonReference.sibling].element

        local control = global.temporaryMaintenanceControl[player_index].byEntity[buttonReference.entity][buttonReference.policyType]
        local changedControls = global.changedControlSettings[player_index]
        ...
        if control.start ~= sliderValue then
            -- save new value
            if not changedControls.byEntity[buttonReference.entity][buttonReference.policyType].start then
                changedControls.total = changedControls.total + 1
            end
            changedControls.byEntity[buttonReference.entity][buttonReference.policyType].start = sliderValue
            confirmButton.enabled = true
        else
            -- same value as default -> remove this from the list
            if changedControls.byEntity[buttonReference.entity][buttonReference.policyType].start ~= nil then
                changedControls.total = changedControls.total - 1
            end
            changedControls.byEntity[buttonReference.entity][buttonReference.policyType].start = nil
        ...
        local sibling_sliderValue = replacementAgeGetSliderValue(sibling.slider_value)

        if (sibling_sliderValue < sliderValue and sibling_sliderValue > 0) or (sliderValue == 0 and sibling_sliderValue > 0) then
        ...
        local label = global.userInterface[player_index].labelReference[buttonReference.effectLabelName]
        local noReplacementLabel = global.userInterface[player_index].labelReference[buttonReference.effectLabelName.."-no-replacement"]
        local icon = global.userInterface[player_index].iconReference[buttonReference.effectLabelName.."-icon"]
        local warningArrow = global.userInterface[player_index].iconReference[buttonReference.effectLabelName.."-warning-arrow"]
        local warningIcon_1 = global.userInterface[player_index].iconReference[buttonReference.effectLabelName.."-warning-icon-1"]
        local warningLabel_1 = global.userInterface[player_index].labelReference[buttonReference.effectLabelName.."-warning-1"]
        local warningIcon_2 = global.userInterface[player_index].iconReference[buttonReference.effectLabelName.."-warning-icon-2"]
        local warningLabel_2 = global.userInterface[player_index].labelReference[buttonReference.effectLabelName.."-warning-2"]
        local warningIcon_3 = global.userInterface[player_index].iconReference[buttonReference.effectLabelName.."-warning-icon-3"]

        if label.valid then
            local valueStart = sliderValue
            local valueLimit = replacementAgeGetSliderValue(sibling.slider_value)
            local caption = ""
            if valueStart < valueLimit then
                caption = format_percentage(valueStart / 100, 0.01).." … "..format_percentage(valueLimit / 100, 0.01)
            elseif valueStart == valueLimit then
        ...
        if sliderValue > 0 then
            local machineFailureRateIndex = calculateFailureRate(const.replacementAge) -- default value
            ...
            if warningIcon_3.valid then
                warningIcon_3.sprite = "warning-icon"
                warningIcon_3.tooltip = {"gui.mm-tooltip-warning-operation-age"}
            end
        else
            machineFailureRate = {"gui.mm-no-replacement-max-failure"}
            machineReplacementCost = - 1
...
function mmGUI.updateForcedReplacementPolicySlider(element, player_index)
    ...
    if buttonReference then
        ...
        if control.limit ~= sliderValue then
            -- save new value
            if not changedControls.byEntity[buttonReference.entity][buttonReference.policyType].limit then
                changedControls.total = changedControls.total + 1
            end
            changedControls.byEntity[buttonReference.entity][buttonReference.policyType].limit = sliderValue
            confirmButton.enabled = true
        else
            -- same value as default -> remove this from the list
            if changedControls.byEntity[buttonReference.entity][buttonReference.policyType].limit ~= nil then
                changedControls.total = changedControls.total - 1
            end
            changedControls.byEntity[buttonReference.entity][buttonReference.policyType].limit = nil
        ...
        if (sibling_sliderValue > sliderValue and sliderValue > 0) or (sibling_sliderValue == 0 and sliderValue > 0) then
        ...
        if label.valid then
            local valueStart = replacementAgeGetSliderValue(sibling.slider_value)
...
function mmGUI.toggleItemPolicyButton(element, player_index)
    ...
    if bR then
        local control = global.temporaryMaintenanceControl[player_index].byEntity[bR.entity][bR.policyType]
        local changedControls = global.changedControlSettings[player_index]
        local confirmButton = global.userInterface[player_index].buttonReference["maintenanceMadness-window-footer-confirmButton"]
        ...
        if changedControls.byEntity[bR.entity][bR.policyType][bR.item].enabled == nil then
            buttonEnabled = not control[bR.item].enabled
        else
            buttonEnabled = not changedControls.byEntity[bR.entity][bR.policyType][bR.item].enabled
        end
        ...
        if control[bR.item].enabled ~= buttonEnabled then
            ...
            changedControls.byEntity[bR.entity][bR.policyType][bR.item].enabled = buttonEnabled
            confirmButton.enabled = true
        else
            -- same value as default -> remove this from the list
            if changedControls.byEntity[bR.entity][bR.policyType][bR.item] ~= nil then
                changedControls.total = changedControls.total - 1
            end
        ...
        local label = global.userInterface[player_index].labelReference[bR.effectLabelName]
        local icon = global.userInterface[player_index].iconReference[bR.effectLabelName.."-icon"]
        local warningArrow = global.userInterface[player_index].iconReference[bR.effectLabelName.."-warning-arrow"]
        local warningIcon = global.userInterface[player_index].iconReference[bR.effectLabelName.."-warning-icon"]
        local warningLabel = global.userInterface[player_index].labelReference[bR.effectLabelName.."-warning"]
        local itemData

        local itemSpritePathG, itemSpritePath, warningIconSpritePath
        if bR.policyType == "maintenance" and label.valid then
            itemData = global.entitiesWithMROenabled[bR.entity][bR.policyType][const.maintenanceLevel]
            itemSpritePathG = "maintenance-needed-grey-icon"
            itemSpritePath = "maintenance-needed-icon"
            warningIconSpritePath = "item/mm-scrapped-"..bR.entity
        elseif bR.policyType == "repair" and label.valid then
            itemData = global.entitiesWithMROenabled[bR.entity][bR.policyType][const.maintenanceLevel].secondary
            itemSpritePathG = "repair-in-progress-gray-icon"
            itemSpritePath = "repair-in-progress-icon"
        ...
        if label.valid then
            for itemName, item in pairs(control) do
                if changedControls.byEntity[bR.entity][bR.policyType][itemName] then
                    if changedControls.byEntity[bR.entity][bR.policyType][itemName].enabled then
                        newEffectPercentage = newEffectPercentage + itemData[itemName].expectedEffect
                    else
        ...
        if (mceil(100000 * newEffectPercentage) / 100000) < 1 then
            ...
            if bR.policyType == "maintenance" then
                warningPercent = 1/(1/(((1-newEffectPercentage)*(const.maxMissedMaintenanceMalus/const.baseAgeing)+1)-1))
            elseif bR.policyType == "repair" then
                warningPercent = (1-newEffectPercentage)/(1/((1/const.baseRepairEffectivity)-1))
            end
            if icon.valid then
    ...
    if nameStrings["maintenanceMadness"] then
        if nameStrings["mainButton"] or nameStrings["closeButton"] then
            if nameStrings["confirmationDialog"] then
                ...
                if global.changedControlSettings[player_index] and global.changedControlSettings[player_index].total > 0 then
                    toggleConfirmationDialog(player_index)
                else
        ...
        elseif nameStrings["discardChangesButton"] then
            discardChangedMaintenanceControlSettings(player_index)
            mmGUI.toggleMasterPanel(player_index)
        elseif nameStrings["resetButton"] or nameStrings["quitResetConfirmationDialog"] then
...
function mmGUI.processChangedTab(event)
    ...
    if nameStrings["maintenanceMadness"] then
        ...
        updateLegendInformation(player_index, element.selected_tab_index, ui.hideLegend)
    end
end
