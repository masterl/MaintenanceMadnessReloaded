...
local function initMaintenanceDemandsTable( entityName, timeFactor )
    ...
    local source = entityMaintencanceDemandByName[entityName] or
                       entityMaintenanceDemandByType[game.entity_prototypes[entityName]
                           .type] -- entity specification > type specification
    for level, costFactor in pairs( const.costFactor ) do
        ...
        for _, ingredient in pairs( source ) do
            ...
            if ingredient.type ~= 'fluid' then
                ...
                resultTable[level][itemName].probability =
                    ingredient.probability
                resultTable[level][itemName].min_amount = mceil( amount *
                                                                     costFactor *
                                                                     timeFactor /
                                                                     2 ) -- min 50% maintenance demand
                resultTable[level][itemName].max_amount = mceil( amount *
                                                                     costFactor *
                                                                     timeFactor *
                                                                     1, 5 ) -- max 150% maintenance demand
            end
        end
        ...
        for _, ingredient in pairs( resultTable[level] ) do
            ...
            ingredient.expectedDemand = (ingredient.max_amount +
                                            ingredient.min_amount) / 2 *
                                            ingredient.probability
        end
    end
...
local function initRepairDemandsTable( entityName, timeFactor )
    ...
    local defaultIngredients =
        entityDefaultRepairDemandByType[game.entity_prototypes[entityName].type]
    for level, costFactor in pairs( const.costFactor ) do
        resultTable[level] = {}
        ...
        for _, ingredient in pairs( ingredients ) do
            ...
            if ingredient.type ~= 'fluid' then
                ...
                local prob = mmin( (amount ^ 2 / 100) * costFactor * timeFactor,
                                   0.5 ) -- ingredients in small amounts are often expensive, therefore the probability for using them as spare parts is lowered
                ...
                resultTable[level].secondary[itemName].min_amount = mceil(
                                                                        amount *
                                                                            costFactor *
                                                                            timeFactor *
                                                                            repairAmountMin ) -- default: min 2,5% of construction cost
                resultTable[level].secondary[itemName].max_amount = mceil(
                                                                        amount *
                                                                            costFactor *
                                                                            timeFactor *
                                                                            repairAmountMax ) -- default: max 7,5% of construction cost
            end
        end
        ...
        -- calculate expected demand and effect (used for gui purposes) -- only approximative!
        local ingredientsCount =
            mm_util.getLength( resultTable[level].secondary )
        ...
        for _, ingredient in pairs( resultTable[level].secondary ) do
            if ingredient.defaultItem then
                ...
                ingredient.expectedDemand =
                    (ingredient.max_amount + ingredient.min_amount) / 2 *
                        invProbProduct
            else
                ingredient.expectedEffect = ingredient.probability / probSum
                ingredient.expectedDemand =
            ...
            expectedEffectChecksum = expectedEffectChecksum +
                                         ingredient.expectedEffect
        end
        -- make sure that the sum of all effects is actually 1. If not, correct the effect value of the default item.
        local sumCorrectionItemName = defaultIngredients[#defaultIngredients]
                                          .name or
                                          defaultIngredients[#defaultIngredients][1]
        if const.debug then
            log( 'Effect sum = ' .. expectedEffectChecksum )
            log( 'Adjusted effect sum for ' .. entityName .. ' by ' ..
                     (1 - expectedEffectChecksum) )
        end
        resultTable[level].secondary[sumCorrectionItemName].expectedEffect =
            resultTable[level].secondary[sumCorrectionItemName].expectedEffect -
                expectedEffectChecksum + 1
        --[[
local n = 3
...
for k, val in pairs(result) do
    print(val/n)
        ...
        local healthFactor = mceil(
                                 game.entity_prototypes[entityName].max_health *
                                     costFactor * timeFactor / 100 ) -- sturdier entities need more repair packs
        local itemTypes = mceil( mm_util.getLength( resultTable[level] ) *
                                     costFactor * timeFactor / 2 ) -- every two different items required for construction, one additional repair pack is needed
        ...
        resultTable[level].primary[repairPackName].max_amount = mceil(
                                                                    (itemTypes +
                                                                        healthFactor) )
    end
    -- log(serpent.block(resultTable, {maxlevel= 4}))
...
local function initAuxTables()
    ...
    for _, entity in pairs( game.entity_prototypes ) do
        ...
        if entityTypesWithMROenabled[entity.type] ~= nil then
            ...
            if game.item_prototypes[entity.name] and
                game.recipe_prototypes[entity.name] and not ignore[entity.name] then
                ...
                newElement.maintenance =
                    initMaintenanceDemandsTable( entity.name, timeFactor )
                newElement.repair = initRepairDemandsTable( entity.name,
                                                            timeFactor )
                newElement.scrap = { ['mm-scrapped-' .. entity.name] = 1 }
                newElement.replacement = { [entity.name] = 1 }
                newElement.flyingTextShiftY =
                    entity.collision_box.left_top.y / 2 -- this will place the flying text more on top of the entity
...
function init_global()
    ...
    global.temporaryMaintenanceControl =
        global.temporaryMaintenanceControl or {}
    ...
    for item, itemData in pairs( source ) do
        if itemData.defaultItem then
            if not policy or policy[item].enabled then
                defaultTable[item] = rand( itemData.min_amount,
                                           itemData.max_amount )
                defaultTableReal[item] = defaultTable[item]
            else
                defaultTableReal[item] =
                    rand( itemData.min_amount, itemData.max_amount )
            end
        elseif itemData.probability == 1 or rand() < itemData.probability then
            if not policy or policy[item].enabled then
                resultTable[item] = rand( itemData.min_amount,
                                          itemData.max_amount )
                resultTableReal[item] = resultTable[item]
            else
                resultTableReal[item] = rand( itemData.min_amount,
                                              itemData.max_amount )
            end
        end
    end
...
local function checkCompletedDemands( chest, requestedItems )
    ...
    local p = 0.00
    local deliveredItems = chest.get_inventory( defines.inventory.chest )
                               .get_contents()
...
local function scheduleNextEvent( eventTick, elementData )
    if global.eventScheduleMRO[eventTick] == nil then
        ...
        global.eventScheduleMRO[eventTick][#global.eventScheduleMRO[eventTick] +
            1] = elementData
    end
end
...
function swapOldMachine( entity )
    ...
    if newOperableEntity.valid then
        ...
        if machine.to_be_deconstructed( machine.force ) then
            newOperableEntity.order_deconstruction( machine.force )
        end
        ...
        global.monitoredEntities[newOperableEntity.unit_number] =
            newtargetReference
        targetReference =
            global.monitoredEntities[newOperableEntity.unit_number]
...
local function collisionTest( rec1, rec2 )
    ...
    local A2x = rec1.right_bottom.x or rec1.right_bottom[1] or rec1[2].x or
                    rec1[2][1]
    local A2y = rec1.right_bottom.y or rec1.right_bottom[2] or rec1[2].y or
                    rec1[2][2]
    ...
    local B2x = rec2.right_bottom.x or rec2.right_bottom[1] or rec2[2].x or
                    rec2[2][1]
    local B2y = rec2.right_bottom.y or rec2.right_bottom[2] or rec2[2].y or
                    rec2[2][2]
...
function getNeighboringMachines(entity,
                                byPlayerIndex,
                                customUnitName,
                                customAoE )
    local areaOfEffect
    ...
    local searchArea = {
        left_top = { position.x - quareRadius, position.y - quareRadius },
        right_bottom = { position.x + quareRadius, position.y + quareRadius }
    }
    local surroundingEntities = entity.surface.find_entities_filtered {
        area = searchArea,
        force = entity.force,
    ...
    for _, candidate in pairs( surroundingEntities ) do
        if candidate.unit_number and
            global.monitoredEntities[candidate.unit_number] then
...
local function getNeighboringMaintenanceUnits( entityID )
    ...
    local quareRadius = mmax( dimensions.height, dimensions.width ) / 2 +
                            const.mUnitMaxRadius
    local position = entity.position
    local searchArea = {
        left_top = { position.x - quareRadius, position.y - quareRadius },
        right_bottom = { position.x + quareRadius, position.y + quareRadius }
    ...
    for _, candidate in pairs( surroundingEntities ) do
        if candidate.unit_number and
            global.maintenanceUnits[candidate.unit_number] then
            local areaOfEffect = global.maintenanceUnits[candidate.unit_number]
                                     .areaOfEffect
...
local function createHiddenChest( machineData, machineProperties )
    ...
    if not machineProperties.square and
        (machine.direction == defines.direction.east or machine.direction ==
            defines.direction.west) then
    ...
    elseif type == 'boiler' then
        ...
        if burner then
            if burner.remaining_burning_fuel ~= lastValue then
                global.monitoredEntities[entityID].activityData =
...
local function initLifeCycle( entity, startMaintenanceInstantly )
    if entity and entity.valid then
        ...
        if startMaintenanceInstantly then
            ...
            nextTick = rand( mfloor( const.MTBM / 10 ),
                             mfloor( const.MTBM * 1, 5 * bonusCycles ) )
            eventTick = game.tick +
                            mfloor( entityProperties.timeFactor * nextTick )
            bonusCycles = 0
        else
            nextTick = rand( mfloor( const.MTBM / 2 ),
                             mfloor( const.MTBM * 1, 5 ) )
            eventTick = game.tick +
                            mfloor( entityProperties.timeFactor * nextTick )
        end
        ...
        global.monitoredEntities[entityID].connectedMUnits =
...
local function setAreaOfEffect( boundingBox, radius )
    local A1x = boundingBox.left_top.x or boundingBox.left_top[1] or
                    boundingBox[1].x or boundingBox[1][1]
    local A1y = boundingBox.left_top.y or boundingBox.left_top[2] or
                    boundingBox[1].y or boundingBox[1][2]
    local A2x = boundingBox.right_bottom.x or boundingBox.right_bottom[1] or
                    boundingBox[2].x or boundingBox[2][1]
    local A2y = boundingBox.right_bottom.y or boundingBox.right_bottom[2] or
                    boundingBox[2].y or boundingBox[2][2]
    return {
        left_top = { x = A1x - radius, y = A1y - radius },
        right_bottom = { x = A2x + radius, y = A2y + radius }
    }
end
...
local function initMaintenanceUnit( entity )
    if entity and entity.valid then
        ...
        local nextTick = rand(
                             mfloor( const.MTTM / (2 * properties.updateRate) ),
                             mfloor( const.MTTM * (1.5 / properties.updateRate) ) ) -- update rate is dependent on MTTM
        local eventTick = game.tick + nextTick

        scheduleNextEvent( eventTick,
                           { id = entityID, type = 'maintenance-unit' } )

        global.maintenanceUnits[entityID] = {
            entity = entity,
            areaOfEffect = setAreaOfEffect( entity.bounding_box,
                                            properties.radius ),
...
local function startMaintenance( entityID )
    ...
    if machine and machine.valid then
        ...
        targetReference.connectedChest =
            createHiddenChest( targetReference, machineProperties )
        local targetChest = targetReference.connectedChest

        local requestedItems, actualRequestedItems = calculateDemands(
                                                         global.maintenanceControl[machine.force
                                                             .index].byEntity[machine.name]
                                                             .maintenance,
                                                         machineProperties.maintenance[const.maintenanceLevel] )
        if mm_util.getLength( requestedItems ) > 0 then
            targetReference.connectedProxy =
                machine.surface.create_entity {
        ...
        local checkForCompletionTick = game.tick +
                                           (rand( mfloor( const.MTTM / 2 ),
                                                  mfloor( const.MTTM * 1, 5 ) )) -- this value is not multiplied with timeFactor!
        targetReference.nextEventTick = checkForCompletionTick

        global.monitoredEntities[entityID] = targetReference
        scheduleNextEvent( checkForCompletionTick,
                           { id = entityID, type = 'machine' } )
...
local function finishMaintenance( entityID )
    ...
    if machine and machine.valid then
        ...
        if targetChest and targetChest.valid then
            maintenanceCompletion = checkCompletedDemands( targetChest,
                                                           targetReference.requestedItems )
            if targetReference.connectedMUnits ~= nil then
                local forceID = machine.force.index
                local forceBonus = global.forceBonus[forceID] and
                                       global.forceBonus[forceID]
                                           .basicMaintenanceBonus or 0
                -- apply force specific tech bonus
                maintenanceCompletion = forceBonus + maintenanceCompletion *
                                            (1 - forceBonus)
            end
            missedMaintenanceMalus = missedMaintenanceMalus *
                                         (1 - maintenanceCompletion)
        ...
        targetReference.serviceLevel = (targetReference.serviceLevel *
                                           (targetReference.serviceRequests - 1)) /
                                           targetReference.serviceRequests +
                                           maintenanceCompletion /
                                           targetReference.serviceRequests

        -- increase ageing -> increased probability for failures
        local newAgeing =
            targetReference.ageing + machineProperties.timeFactor *
                (const.baseAgeing + missedMaintenanceMalus)
        targetReference.ageing = newAgeing

        if const.showText then
            local color = mm_util.calculateTextColor(
                              textColors.maintenance.min,
                              textColors.maintenance.delta,
                              maintenanceCompletion )
            local textPos = {
                x = machine.position.x,
                y = machine.position.y + machineProperties.flyingTextShiftY
            }
            machine.surface.create_entity {
                name = 'mm-flying-text',
                position = textPos,
                text = {
                    'mm-flying-text-maintenance-finished',
                    math.floor( maintenanceCompletion * 100 )
                },
                color = color,
                force = machine.force
            }
        ...
        local eventTick = game.tick + mfloor(
                              machineProperties.timeFactor *
                                  (rand( mfloor( const.MTBM / 2 ),
                                         mfloor( const.MTBM * 1.5 ) )) )
...
local function causeFailure( entityID, entityActive )
    ...
    if machine and machine.valid then
        local damageFactor = rand() * const.maxDamageFactor *
                                 (targetReference.ageing / const.maxAge)
        ...
        if not entityActive then
            ...
            damageFactor = const.baseDamageFactor + damageFactor *
                               const.idleFailureDamageFactor
        else
            damageFactor = const.baseDamageFactor + damageFactor
        end
        machine.damage( mfloor( machine.prototype.max_health * damageFactor ),
                        force )
        ...
        if machine.type == 'solar-panel' or machine.type == 'accumulator' and
            (#machine.circuit_connected_entities.red +
                #machine.circuit_connected_entities.green) == 0 then
            ...
            if machine.to_be_deconstructed( force ) then
                newFaultyEntity.order_deconstruction( machine.force )
            end
            local newtargetReference = targetReference
            ...
            newtargetReference.ageing = targetReference.ageing
            newtargetReference.status = targetReference.status
            global.monitoredEntities[newFaultyEntity.unit_number] =
                newtargetReference
            targetReference =
                global.monitoredEntities[newFaultyEntity.unit_number]
            global.monitoredEntities[entityID] = nil
        ...
        local requestedPrimaryItems = calculateDemands( false,
                                                        machineProperties.repair[const.maintenanceLevel]
                                                            .primary )
        targetReference.connectedProxy =
            machine.surface.create_entity {
                name = 'mm-repair-request-proxy',
                position = targetChest.position,
                target = targetChest,
                force = targetChest.force,
                modules = requestedPrimaryItems
            }
        targetReference.requestedItems = requestedPrimaryItems
        machine.surface.play_sound {
            path = 'mm-machine-failure-sound',
            position = machine.position,
            volume_modifier = 0.75
        }

        local timeToRepair = mfloor(
                                 machineProperties.timeFactor * (const.MTTR +
                                     (const.MTTR * const.repairTimeModifier *
                                         targetReference.ageing / const.maxAge)) )
        if const.lowTechModifierEnabled then
            -- early game bonus: reduced repair time
            timeToRepair = mfloor( timeToRepair *
                                       global.lowTechModifier[force.index]
                                           .factor )
        end
        ...
        if targetReference.connectedMUnits ~= nil then
            ...
            targetReference.repairEffectivity =
                targetReference.repairEffectivity +
                    (global.forceBonus[force.index] and
                        global.forceBonus[force.index].basicRepairSpeedBonus or
                        0)
        end
        ...
        if machine.type == 'solar-panel' or machine.type == 'accumulator' then
            if const.showMIPsignForSolar then
                targetReference.renderedIconID =
                    rendering.draw_sprite {
                        sprite = 'repair-pending-icon',
                        render_layer = 'entity-info-icon',
                        target = targetReference.connectedChest,
                        target_offset = machine.prototype.alert_icon_shift or
                            { 0, 0 },
        ...
        if const.showText then
            ...
            machine.surface.create_entity {
                name = 'mm-flying-text',
                position = textPos,
                text = { 'mm-flying-text-failure' },
                color = textColors.failure,
                force = machine.force
            }
        end

        local tickDelayBetweenRepairChecks = mfloor(
                                                 machineProperties.timeFactor *
                                                     const.tickDelayCheckRepairState )
...
local function startRepair( entityID )
    ...
    if machine and machine.valid then
        ...
        if targetChest and targetChest.valid then
            if checkCompletedDemands( targetChest,
                                      targetReference.requestedItems ) >= 1 then
                ...
                local requestedSecondaryItems, actualRequestedSecondaryItems =
                    calculateDemands( global.maintenanceControl[machine.force
                                          .index].byEntity[machineName].repair,
                                      machineProperties.repair[const.maintenanceLevel]
                                          .secondary )
                if mm_util.getLength( requestedSecondaryItems ) > 0 then
                    targetReference.connectedProxy = machine.surface
                                                         .create_entity {
                    ...
                    -- add an icon that is hidden until the requester icon is removed by fulfilling the request
                    if machine.type == 'solar-panel' or machine.type ==
                        'accumulator' then
                        if const.showMIPsignForSolar then
                            rendering.draw_animation {
                                ...
                                render_layer = 'entity-info-icon',
                                target = targetReference.connectedChest,
                                target_offset = machine.prototype
                                    .alert_icon_shift or { 0, 0 },
                                surface = machine.surface,
                                forces = { machine.force }
                            }
                        end
                ...
                if not targetReference.connectedProxy.valid then
                    targetReference.connectedProxy = machine.surface
                        ...
                        target = targetChest,
                        force = targetChest.force,
                        modules = targetReference.requestedItems
                    }
                end
                ...
                if const.showText and rand() < const.showDefectRemainderProb then
                    local textPos = {
                        ...
                        y = machine.position.y +
                            machineProperties.flyingTextShiftY
                    }
                    machine.surface.create_entity {
                        name = 'mm-flying-text',
                        position = textPos,
                        text = { 'mm-flying-text-failure-ongoing' },
                        color = textColors.failure2,
                        force = machine.force
                    }
                end
...
local function checkRepairState( entityID )
    ...
    if machine and machine.valid then
        ...
        if targetReference.connectedMUnits ~= nil then
            local forceID = machine.force.index
            -- if connected to a maintenance unit, check if force tech boni have to be applied
            baseRepairEffectivity = baseRepairEffectivity +
                                        (global.forceBonus[forceID] and
                                            global.forceBonus[forceID]
                                                .basicRepairSpeedBonus or 0)
        end
        if targetChest and targetChest.valid then
            -- check if / how many repair materials have been delivered
            local repairMaterialSupplyRate =
                checkCompletedDemands( targetChest,
                                       targetReference.requestedItems )
            local totalRepairEffectivity =
                baseRepairEffectivity + (1 - baseRepairEffectivity) *
                    repairMaterialSupplyRate

            local effectiveRepairTime = mfloor(
                                            targetReference.repairTimeElapsed *
                                                totalRepairEffectivity )
            local remainingRepairTime = targetReference.timeToRepair -
                                            effectiveRepairTime
            local estimatedTimeToComplete = mceil(
                                                remainingRepairTime /
                                                    totalRepairEffectivity )
            ...
            if remainingRepairTime <= 0 then
                ...
                targetReference.ageing =
                    targetReference.ageing + machineProperties.timeFactor *
                        const.repairAgeMalus
                targetReference.status = 'operating'
                machine.active = true
                ...
                if const.showText then
                    ...
                    machine.surface.create_entity {
                        name = 'mm-flying-text',
                        position = textPos,
                        text = { 'mm-flying-text-machine-fixed' },
                        color = textColors.repair,
                        force = machine.force
                    }
                end
                machine.surface.play_sound {
                    path = 'mm-machine-fixed-sound',
                    position = machine.position,
                    volume_modifier = 0.75
                }

                eventTick = game.tick + mfloor(
                                machineProperties.timeFactor *
                                    rand( mfloor( const.MTBM / 2 ),
                                          mfloor( const.MTBM * 1.5 ) ) )
            else
                ...
                local nextCheck = mceil( mmin(
                                             mmax( remainingRepairTime,
                                                   const.tickDelayCheckRepairState ),
...
function requestReplacementOfOldMachine( entityID )
    ...
    if machine and machine.valid and targetReference.status == 'operating' then
        ...
        targetReference.connectedProxy =
            machine.surface.create_entity {
                name = 'mm-replacement-request-proxy',
                position = targetChest.position,
                target = targetChest,
                force = targetChest.force,
                modules = requestedItems
...
local function checkForScrapReturn( entityID )
    ...
    if machine and machine.valid then
        if targetReference.producedItems and targetReference.producedItems == 0 then
            if (const.maxAgeForFreeReturn * const.maxAge) >
                targetReference.ageing then
                return nil
            end
        end
        if rand( 0, const.maxAge ) <
            (targetReference.ageing * const.scrapReturnRate) then
...
local function checkReplacementSuccess( entityID )
    ...
    if machine and machine.valid then
        local targetChest = targetReference.connectedChest
        local replacementSupplied = checkCompletedDemands( targetChest,
                                                           targetReference.requestedItems )
        if replacementSupplied >= 1 then
            ...
            if scrap ~= nil then
                for item, amount in pairs( scrap ) do
                    returnedItem = machine.surface.create_entity {
                        name = 'item-on-ground',
                        position = machine.position,
                        stack = { name = item, count = amount },
                        force = machine.force
                    }
                    if returnedItem.valid then
                        returnedItem.order_deconstruction( machine.force )
                        machine.force.item_production_statistics.on_flow( item,
                                                                          amount )
                    end
                end
            end
...
function forceReplacementOfOldMachine( entityID )
    ...
    if machine and machine.valid and targetReference.status == 'operating' then
        ...
        targetReference.connectedProxy =
            machine.surface.create_entity {
                name = 'mm-forced-replacement-request-proxy',
                position = targetChest.position,
                target = targetChest,
                force = targetChest.force,
                modules = requestedItems
        ...
        if global.eventScheduleMRO and
            global.eventScheduleMRO[targetReference.nextEventTick] ~= nil then
            for index, element in ipairs(
                                      global.eventScheduleMRO[targetReference.nextEventTick] ) do
                if entityID == element.id then
                    global.eventScheduleMRO[targetReference.nextEventTick][index] =
                        nil
                end
            end
        end
        ...
        if const.showText then
            ...
            machine.surface.create_entity {
                ...
                position = textPos,
                text = {
                    'mm-flying-text-replacement-max-operation-time-exceeded'
                },
                color = textColors.failure,
                force = machine.force
            }
        ...
        local tickDelayBetweenReplacmentChecks = mfloor(
                                                     machineProperties.timeFactor *
                                                         const.tickDelayCheckRepairState )
...
local function checkForcedReplacementSuccess( entityID )
    ...
    if machine and machine.valid then
        ...
        local targetChest = targetReference.connectedChest
        local nextEventTick
        if checkCompletedDemands( targetChest, targetReference.requestedItems ) >=
            1 then
            -- when all materials have been delivered, repair starts
            postItemsAsConsumption( targetChest )
            global.activeChests[targetChest.unit_number] = nil
            ...
            if const.showText then
                ...
                machine.surface.create_entity {
                    name = 'mm-flying-text',
                    position = textPos,
                    text = { 'mm-flying-text-replacement-done' },
                    color = textColors.repair,
                    force = machine.force
                }
            end
            ...
            nextEventTick = game.tick + mfloor(
                                machineProperties.timeFactor *
                                    rand( mfloor( const.MTBM / 2 ),
                                          mfloor( const.MTBM * 1.5 ) ) )
        else
            ...
            if const.showText and rand() < const.showDefectRemainderProb then
                ...
                machine.surface.create_entity {
                    name = 'mm-flying-text',
                    position = textPos,
                    text = { 'mm-flying-text-replacement-not-yet-delivered' },
                    color = textColors.failure2,
                    force = machine.force
                }
            end
...
local function stopMaintenanceCycle( entityID, showText )
    ...
    if machine and machine.valid then
        ...
        if status == 'malfunction' or status == 'repair-ongoing' or status ==
            'replacement-required' then
            ...
            if const.showText and showText ~= false then
                local textPos = {
                    x = machine.position.x,
                    y = machine.position.y +
                        global.entitiesWithMROenabled[machineName]
                            .flyingTextShiftY
                }
...
local function startMaintenanceCycleForExistingEntities( names )
    for _, surface in pairs( game.surfaces ) do
        for _, entity in
            pairs( surface.find_entities_filtered { name = names } ) do
...
local function transferItemsIfAvailable(targetInventory,
                                        sourceInventory,
                                        requested,
                                        item_request_proxy )
    ...
    for requestedItem, requestedAmount in pairs( requested ) do
        ...
        if sourceContents[requestedItem] then
            ...
            if sourceContents[requestedItem] >= requestedAmount then
                sourceInventory.remove( {
                    name = requestedItem,
                    count = requestedAmount
                } )
                targetInventory.insert( {
                    name = requestedItem,
                    count = requestedAmount
                } )
            elseif sourceContents[requestedItem] > 0 then
                sourceInventory.remove( {
                    name = requestedItem,
                    count = sourceContents[requestedItem]
                } )
                targetInventory.insert( {
                    name = requestedItem,
                    count = sourceContents[requestedItem]
                } )
                remaining[requestedItem] =
                    requested[requestedItem] - sourceContents[requestedItem]
...
local function checkRepairCapacity( inventory, repairRequested )
    ...
    for _, repairItem in pairs( const.repairItemPriority ) do
        if contents[repairItem] then
            ...
            local possibleRepairItemDrain = mmin(
    ...
    if actualRepairCapacity < repairRequested then
        for itemName, _ in pairs( contents ) do
            ...
            if stack.type == 'repair-tool' and
                const.repairItemPriority[stack.name] == nil then
                local toolSpeed = stack.prototype.speed
                local drainAmount = repairRequested / toolSpeed
                local possibleRepairItemDrain =
                    mmin( stack.durability * stack.count, drainAmount )
                local possibleRepairCapacity =
                    possibleRepairItemDrain * toolSpeed
                if possibleRepairCapacity > actualRepairCapacity then
...
local function processMaintenanceUnit( entityID )
    if entityID ~= nil and global.maintenanceUnits[entityID] ~= nil then
            ...
            if mUnitEEI.energy > 0 then
                for _, machineID in pairs( mUnitReference.connectedMachines ) do
                    ...
                    if machineData ~= nil then
                        if machineData.connectedProxy ~= nil and
                            machineData.connectedProxy.valid then
                            local requested =
                                machineData.connectedProxy.item_requests
                            local target =
                                machineData.connectedChest.get_inventory(
                                    defines.inventory.chest )
                            if transferItemsIfAvailable( target, storage,
                                                         requested,
                                                         machineData.connectedProxy ) ==
                                'fulfilled' then
                                machineData.connectedProxy.destroy()
                            end
                        end
                        if machineData.entity ~= nil and
                            machineData.entity.valid then
                            local health = machineData.entity.health
                            local maxHealth =
                                machineData.entity.prototype.max_health
                            if health < maxHealth then
                                local avgRepairRate =
                                    properties.repairRate * maxHealth
                                local repairNeeded =
                                    mceil( mmin( maxHealth - health, rand(
                                                     mfloor( avgRepairRate * 0.5 ),
                                                     mceil( avgRepairRate * 1.5 ) ) ) )
                                local repairItemUsed, repairItemDrained,
                                      actualRepairCapacity =
                                    checkRepairCapacity( storage, repairNeeded )
                                if repairItemUsed ~= nil then
                                    storage.find_item_stack( repairItemUsed )
                                        .drain_durability( repairItemDrained )
        ...
        local nextTick = rand(
                             mfloor( const.MTTM / (2 * properties.updateRate) ),
                             mfloor( const.MTTM * (1.5 / properties.updateRate) ) ) -- update rate is dependent on MTBM
        local eventTick = game.tick + nextTick
...
local function processManualMaintenance( playerID )
    if playerID ~= nil and game.players[playerID] ~= nil then
        ...
        local missing_items_msg = {
            ['maintenance'] = {
                'mm-flying-text-manual-maintenance-no-spareparts'
            },
            ...
            ['awaiting-replacement'] = {
                'mm-flying-text-manual-maintenance-no-replacement'
            },
        ...
        if selected ~= nil then
            ...
            elseif selected.prototype.type == 'item-request-proxy' and
                selected.proxy_target ~= nil and
                selected.proxy_target.unit_number ~= nil then
                uID = selected.proxy_target.unit_number
                if uID ~= nil and global.activeChests[uID] ~= nil then
                    machineData =
                        global.monitoredEntities[global.activeChests[uID]]
                end
            end
            if machineData ~= nil then
                if machineData.connectedProxy ~= nil and
                    ...
                    if player.can_reach_entity( selected ) then
                        local requested =
                            machineData.connectedProxy.item_requests
                        local target = machineData.connectedChest
                                           .get_inventory( defines.inventory
                                                               .chest )
                        local demand = transferItemsIfAvailable( target,
                        ...
                        if missing_items_type ~= false then
                            player.create_local_flying_text {
                                text = missing_items_msg[missing_items_type],
...
function calculateLowTechModifier( force_index, update )
    ...
    local researchedUnitCount = global.lowTechModifier[force_index]
                                    .researchedUnits
    ...
    if update then
        ...
        global.lowTechModifier[force_index].researchedUnits =
            researchedUnitCount
    end

    global.lowTechModifier[force_index].factor =
        (researchedUnitCount / totalUnitCount) * (1 - const.lowTechModifierBase) +
            const.lowTechModifierBase
end
...
function initMaintenanceForExistingSave()
    ...
    initMaintenancePrototypes['accumulator'] =
        settings.global['mm-include-accumulators'].value
    initMaintenancePrototypes['solar-panel'] =
        settings.global['mm-include-solar-panels'].value
    initMaintenancePrototypes['reactor'] =
        settings.global['mm-include-reactors'].value
    initMaintenancePrototypes['boiler'] =
        settings.global['mm-include-boiler-and-generators'].value
    initMaintenancePrototypes['generator'] =
        settings.global['mm-include-boiler-and-generators'].value
    initMaintenancePrototypes['mining-drill'] =
        settings.global['mm-include-miners'].value
    initMaintenancePrototypes['roboport'] =
        settings.global['mm-include-roboports'].value
    initMaintenancePrototypes['beacon'] =
        settings.global['mm-include-beacons'].value
    initMaintenancePrototypes['lab'] = settings.global['mm-include-labs'].value
    if const.maintenanceActivated and
        mm_util.getLength( initMaintenancePrototypes ) > 0 then
        local initEntity = {}
        for _, entity in pairs( game.entity_prototypes ) do
            ...
            if initMaintenancePrototypes[entity.type] then
                if game.item_prototypes[entity.name] and
                    game.recipe_prototypes[entity.name] and
                    string.find( entity.name, '-placement%-entity' ) == nil then
...
commands.add_command( 'maintenance-mod-reload', { 'mm-cmd-reload-help' },
                      function( param )
    ...
    mm_util.notification( {
        'mm-notification-cmd-issued',
        { 'mm-prefix' },
        { 'mm-cmd-reload-completed' }
    } )
end )

commands.add_command( 'maintenance-mod-print-debug', { 'mm-cmd-log-debug' },
                      function( param )
...
script.on_configuration_changed( function( data )
    ...
    if data.mod_changes ~= nil and data.mod_changes['MaintenanceMadness'] ~= nil and
        data.mod_changes['MaintenanceMadness'].old_version == nil then
        -- anounce installation
        mm_util.notification( {
            'mm-notification-midgame-update',
            { 'mm-prefix' },
            data.mod_changes['MaintenanceMadness'].new_version
        } )
        initMaintenanceForExistingSave()
        -- midgame update
    elseif data.mod_changes ~= nil and data.mod_changes['MaintenanceMadness'] ~=
        nil and data.mod_changes['MaintenanceMadness'].old_version ~= nil then
...
script.on_event( {
    ...
    if entity ~= nil and entity.valid then
        ...
        if forcesWithMROdisabled[entity.force.name] ~= true then
            if global.entitiesWithMROenabled[name] ~= nil and
                global.entitiesWithMROenabled[name].active then
...
script.on_event( {
                ...
                for _, connector in pairs( entity.connections ) do
                    for wireType, wires in pairs( connector ) do
                        for _, connectedEntity in pairs( wires ) do
                            ...
                            for connectorID, targetConnectors in pairs(
                                                                     entities[connectedEntity.entity_id]['connections'] ) do
                                if targetConnectors[wireType] ~= nil then
                                    for targetIndex,
                                        targetConnectedEntity in pairs(
                                                                     targetConnectors[wireType] ) do
                                        if targetConnectedEntity.entity_id ==
                                            entity.entity_number then
                                            -- remove this unit from the connections table of this entity and deconstruct table afterwards if necessary
                                            targetConnectors[wireType][targetIndex] =
                                                nil
                                            circuit_connection = true
                                            if mm_util.getLength(
                                                targetConnectors[wireType] ) ==
                                                0 then
                                                targetConnectors[wireType] = nil
                                                if mm_util.getLength(
                                                    entities[connectedEntity.entity_id]['connections'][connectorID] ) ==
                                                    0 then
                                                    entities[connectedEntity.entity_id]['connections'][connectorID] =
                                                        nil
                                                    if mm_util.getLength(
                                                        entities[connectedEntity.entity_id]['connections'] ) ==
                                                        0 then
                                                        entities[connectedEntity.entity_id]['connections'] =
                                                            nil
                                                    end
        ...
        if bp.blueprint_icons then
            ...
            for _, icon in pairs( icons ) do
                if string.sub( icon.signal.name, 1, 10 ) == 'mm-faulty-' then
                    icon.signal.name = string.gsub( icon.signal.name,
                                                    'mm%-faulty%-', '' )
    ...
    if circuit_connection then
        player.print( {
            'mm-notification-tried-to-blueprint-circuit-connected-maintenance-unit',
            { 'mm-prefix' }
        } )
    end
end )
...
script.on_event( defines.events.on_research_finished, function( event )
    ...
    if const.lowTechModifierEnabled and global.lowTechModifier[fi].techs[name] ~=
        nil then
        ...
        if settings.global['mm-enable-low-tech-update-notification'].value then
            mm_util.notification( {
                'mm-notification-with-prefix',
                { 'mm-prefix' },
                { 'mm-notification-lowtech-bonus-update-1' }
            }, force )
            mm_util.notification( {
                'mm-notification-with-prefix',
                { 'mm-prefix' },
                {
                    'mm-notification-lowtech-bonus-update-2',
                    mm_util.round( global.lowTechModifier[fi].factor * 100, 0.1 ),
                    mm_util.round( (old_value) * 100, 0.1 )
                }
            }, force )
        end
    end
    ...
    if s2 ~= nil then
        ...
        if forceBonus == nil then
            forceBonus = {}
        end
        forceBonus.basicMaintenanceBonus =
            (forceBonus.basicMaintenanceBonus or 0) +
                (internalSettings.techBoni['improvedMaintenanceAccess'][event.research
                    .level].basicMaintenanceBonus or 0)
        forceBonus.basicRepairSpeedBonus =
            (forceBonus.basicRepairSpeedBonus or 0) +
                (internalSettings.techBoni['improvedMaintenanceAccess'][event.research
                    .level].basicRepairSpeedBonus or 0)
...
local function buildCursorBox( playerID, target_entity )
    ...
    for i = 1, 4 do
        global.last_selected[playerID][#global.last_selected[playerID] + 1] =
            rendering.draw_sprite {
                ...
                target_offset = {
                    offsetVector[i].x * (dimensions.width / 2),
                    offsetVector[i].y * (dimensions.height / 2)
                },
...
local function update_information_overlay( player_index )
    ...
    if entity and entity.valid then
        local entityID = entity.unit_number
        if entity.prototype.type == 'item-request-proxy' and entity.proxy_target ~=
            nil and entity.proxy_target.valid then
        ...
        if global.monitoredEntities[entityID] then
            ...
            if age < const.replacementAge then
                ...
                color = mm_util.calculateTextColor( textColors.age_phase1.min,
                                                    textColors.age_phase1.delta,
                                                    percentage )
            elseif age < const.maxAge then
                local percentage = (age - const.replacementAge) /
                                       (const.maxAge - const.replacementAge)
                color = mm_util.calculateTextColor( textColors.age_phase2.min,
                                                    textColors.age_phase2.delta,
                                                    percentage )
            ...
            local dimensions = mm_util.getBoundingBoxDimensions(
                                   entity.selection_box )
            if not debug then
                if targetReference.status == 'malfunction' and
                    not targetReference.connectedProxy.valid then
                    ...
                    local estimatedRepairTime = mceil(
                                                    targetReference.nextEventTick -
                                                        game.tick )

                    global.last_selected[playerID][#global.last_selected[playerID] +
                        1] = rendering.draw_text {
                        text = {
                            'mm-status-overlay-repair-pending',
                            format_tick_to_time( estimatedRepairTime )
                        },
                ...
                elseif targetReference.status == 'repair-ongoing' then
                    ...
                    local timeToRepair =
                        targetReference.timeToRepair -
                            mfloor( usedTimeSpan * repairEffectivity )
                    local remainingRepairTime =
                        mceil( timeToRepair / repairEffectivity )
                    local estimatedRepairTime =
                        mmax( remainingTimeSpan, remainingRepairTime )
                    global.last_selected[playerID][#global.last_selected[playerID] +
                        1] = rendering.draw_text {
                        text = {
                            'mm-status-overlay-repair-time',
                            format_tick_to_time( estimatedRepairTime )
                        },
                        surface = player.surface,
                        scale = 1,
                    ...
                    global.last_selected[playerID][#global.last_selected[playerID] +
                        1] = rendering.draw_text {
                        text = {
                            'mm-status-overlay-repair-effectivity',
                            mfloor( repairEffectivity * 100 )
                        },
                        surface = player.surface,
                        scale = 1,
                        target = entity,
                        target_offset = { 0, (dimensions.height / 2) + 1.3 },
                        color = mm_util.calculateTextColor(
                            textColors.maintenance.min,
                            textColors.maintenance.delta, repairEffectivity ),
                        players = { playerID },
                        draw_on_ground = false,
                        alignment = 'center',
                        scale_with_zoom = true,
                ...
                elseif targetReference.serviceRequests > 0 then
                    global.last_selected[playerID][#global.last_selected[playerID] +
                        1] = rendering.draw_text {
                        text = {
                            'mm-status-overlay-service-level',
                            mfloor( targetReference.serviceLevel * 100 )
                        },
                        surface = player.surface,
                        scale = 1,
                        target = entity,
                    ...
                    if targetReference.status == 'awaiting-replacement' or
                        targetReference.status == 'replacement-required' then
                        local targetChest = targetReference.connectedChest
                        if targetChest and targetChest.valid then
                            local replacementSupplied =
                                checkCompletedDemands( targetChest,
                                                       targetReference.requestedItems )
                            if replacementSupplied >= 1 then
                                global.last_selected[playerID][#global.last_selected[playerID] +
                                    ...
                                    rendering.draw_text {
                                        text = {
                                            'mm-status-overlay-replacement-pending'
                                        },
                                        surface = player.surface,
                                        scale = 1,
                                        target = entity,
                ...
                global.last_selected[playerID][#global.last_selected[playerID] +
                    1] = rendering.draw_text {
                    text = 'Total maintenance requests: ' ..
                        tostring( targetReference.serviceRequests ),
                    surface = player.surface,
                    scale = 1,
                    target = entity,
                    target_offset = { 0, (dimensions.height / 2) + 1.3 },
                ...
                global.last_selected[playerID][#global.last_selected[playerID] +
                    ...
                    text = 'Average service level: ' ..
                        tostring( targetReference.serviceLevel ),
                    surface = player.surface,
                    scale = 1,
                    target = entity,
                ...
                global.last_selected[playerID][#global.last_selected[playerID] +
                    ...
                    text = 'Next event in ' ..
                        format_tick_to_time(
                            targetReference.nextEventTick - game.tick ),
                    surface = player.surface,
                    scale = 1,
                    target = entity,
        ...
        elseif global.maintenanceUnits[entityID] then
            ...
            if mUnitEEI and mUnitEEI.valid and mUnitEEI.energy == 0 then
                ...
                global.last_selected[playerID][#global.last_selected[playerID] +
                    1] = rendering.draw_text {
                    text = {
                        'mm-status-overlay-inspected-machines',
                        #mUnitReference.connectedMachines
                    },
                    surface = player.surface,
                    scale = 1,
                    target = entity,
                ...
                global.last_selected[playerID][#global.last_selected[playerID] +
                    1] = rendering.draw_text {
                    text = {
                        'mm-status-overlay-next-inspection',
                        format_tick_to_time(
                            mUnitReference.nextEventTick - game.tick )
                    },
                    surface = player.surface,
                    scale = 1,
                    target = entity,
    ...
    for _, force in pairs( game.forces ) do
        ...
        if global.maintenanceControl[forceID] then
            if global.maintenanceControl[forceID].byEntity then
                for _, entity in pairs( global.maintenanceControl[forceID]
                                            .byEntity ) do
...
script.on_event( defines.events.on_runtime_mod_setting_changed,
    ...
    if event.setting_type == 'runtime-global' then
        ...
        elseif s == 'mm-maintenance-cost' then
            const.maintenanceLevel = settings.global['mm-maintenance-cost']
                                         .value
        ...
        elseif s == 'mm-show-maintenance-in-progress-sign-for-solar' then
            const.showMIPsignForSolar =
                settings.global['mm-show-maintenance-in-progress-sign-for-solar']
                    .value
        elseif s == 'mm-expected-life-time' then
            const.expectedLifeTime = mfloor( 3600 * 60 *
                                                 settings.global['mm-expected-life-time']
                                                     .value ) -- 3600 ticks = 1 min
            recalculateMainParameters = true
        elseif s == 'mm-mean-time-between-maintenance-events' then
            const.MTBM = mfloor( 3600 *
                                     settings.global['mm-mean-time-between-maintenance-events']
                                         .value )
            recalculateMainParameters = true
        elseif s == 'mm-mean-time-to-maintain' then
            const.MTTM = mfloor( 3600 *
                                     settings.global['mm-mean-time-to-maintain']
                                         .value )
            recalculateMainParameters = true
        elseif s == 'mm-mean-time-to-repair' then
            const.MTTR = mfloor( 3600 *
                                     settings.global['mm-mean-time-to-repair']
                                         .value )
            recalculateMainParameters = true
        elseif s == 'mm-include-accumulators' then
            if settings.global['mm-include-accumulators'].value then
                initMaintenancePrototypes['accumulator'] = true
            else
                stopMaintenancePrototypes['accumulator'] = true
            end
        elseif s == 'mm-include-solar-panels' then
            if settings.global['mm-include-solar-panels'].value then
                initMaintenancePrototypes['solar-panel'] = true
            else
                stopMaintenancePrototypes['solar-panel'] = true
            end
        elseif s == 'mm-include-reactors' then
            if settings.global['mm-include-reactors'].value then
                initMaintenancePrototypes['reactor'] = true
            else
                stopMaintenancePrototypes['reactor'] = true
            end
        elseif s == 'mm-include-boiler-and-generators' then
            if settings.global['mm-include-boiler-and-generators'].value then
                initMaintenancePrototypes['boiler'] = true
            else
                stopMaintenancePrototypes['boiler'] = true
            end
            if settings.global['mm-include-boiler-and-generators'].value then
                initMaintenancePrototypes['generator'] = true
            else
                stopMaintenancePrototypes['generator'] = true
            end
        elseif s == 'mm-include-miners' then
            if settings.global['mm-include-miners'].value then
                initMaintenancePrototypes['mining-drill'] = true
            else
                stopMaintenancePrototypes['mining-drill'] = true
            end
        elseif s == 'mm-include-roboports' then
            if settings.global['mm-include-roboports'].value then
                initMaintenancePrototypes['roboport'] = true
            else
                stopMaintenancePrototypes['roboport'] = true
            end
        elseif s == 'mm-include-beacons' then
            if settings.global['mm-include-beacons'].value then
                initMaintenancePrototypes['beacon'] = true
            else
                stopMaintenancePrototypes['beacon'] = true
            end
        elseif s == 'mm-include-labs' then
            if settings.global['mm-include-labs'].value then
                initMaintenancePrototypes['lab'] = true
            else
                stopMaintenancePrototypes['lab'] = true
            end
        elseif s == 'mm-solar-factor' then
            const.solarCycleTimeMultiplier =
                settings.global['mm-solar-factor'].value
        ...
        elseif s == 'mm-replacement-age' then
            ...
            const.replacementAge = settings.global['mm-replacement-age'].value *
                                       (const.maxAge / 100)
        ...
        elseif s == 'mm-wear-reduction-if-idle' then
            const.idleWearReductionFactor =
                settings.global['mm-wear-reduction-if-idle'].value / 100
        elseif s == 'mm-missed-maintenance-malus' then
            const.maxMissedMaintenanceMalus =
                (settings.global['mm-missed-maintenance-malus'].value / 100) *
                    const.baseAgeing
            const.repairAgeMalus = const.baseAgeing -- const.maxMissedMaintenanceMalus * 0,5
        elseif s == 'mm-enable-low-tech-bonus' then
            const.lowTechModifierEnabled =
                settings.global['mm-enable-low-tech-bonus'].value
            for _, force in pairs( game.forces ) do
                local fi = force.index
                if const.lowTechModifierEnabled == true then
                    ...
                    mm_util.notification( {
                        ...
                        { 'mm-prefix' },
                        {
                            'mm-notification-lowtech-bonus-enabled',
                    ...
                    mm_util.notification( {
                        'mm-notification-with-prefix',
                        { 'mm-prefix' },
                        { 'mm-notification-lowtech-bonus-disabled' }
                    }, force )
                    global.lowTechModifier[fi] = {}
                end
            end
        elseif s == 'mm-low-tech-bonus-target-tech' then
            if game.technology_prototypes[settings.global['mm-low-tech-bonus-target-tech']
                .value] == nil then
                mm_util.notification( {
                    'mm-notification-with-prefix',
                    { 'mm-prefix' },
                    {
                        'mm-notification-lowtech-bonus-changed-target-tech-failure',
                        settings.global['mm-low-tech-bonus-target-tech'].value,
                        { 'technology-name.' .. const.lowTechModifierDefault }
                    }
                } )
                const.lowTechModifierTarget = const.lowTechModifierDefault
                settings.global['mm-low-tech-bonus-target-tech'].value =
                    const.lowTechModifierDefault
            else
                const.lowTechModifierTarget =
                    settings.global['mm-low-tech-bonus-target-tech'].value
                if const.lowTechModifierEnabled then
                    for _, force in pairs( game.forces ) do
                        local fi = force.index
                        ...
                        mm_util.notification( {
                            ...
                            { 'mm-prefix' },
                            {
                                'mm-notification-lowtech-bonus-changed-target-tech-success',
                                {
                                    'technology-name.' ..
                                        const.lowTechModifierTarget
                                }
        ...
        if const.maintenanceActivated and
            ...
            for _, entity in pairs( game.entity_prototypes ) do
                ...
                if initMaintenancePrototypes[entity.type] then
                    if game.item_prototypes[entity.name] and
                        game.recipe_prototypes[entity.name] then
                        if not global.entitiesWithMROenabled[entity.name] then
                            global.entitiesWithMROenabled[entity.name] = {}
                        end
                        global.entitiesWithMROenabled[entity.name].active = true
            ...
            if not const.maintenanceActivated then -- mod disabled, initiate instant stop. This may lag the game
                ...
                for _, entity in pairs( game.entity_prototypes ) do
                    ...
                    if stopMaintenancePrototypes[entity.type] then
                        if game.item_prototypes[entity.name] and
                            game.recipe_prototypes[entity.name] and
                            global.entitiesWithMROenabled[entity.name] then
                            global.entitiesWithMROenabled[entity.name].active =
                                false
                ...
                for entityName, entityData in pairs(
                                                  global.entitiesWithMROenabled ) do
                    if stopMaintenancePrototypes[game.entity_prototypes[entityName]
                        .type] then
        ...
        if recalculateMainParameters then
            ...
            const.expectedCyclesPerLifeTime =
                mm_util.standardizeCycleTime( {
                    MTBM = const.MTBM,
                    MTTM = const.MTTM,
                    MTTR = const.MTTR,
                    maxAge = const.maxAge,
            ...
            const.baseAgeing = math.ceil(
                                   100 * const.maxAge /
                                       const.expectedCyclesPerLifeTime ) / 100
            const.maxMissedMaintenanceMalus =
                (settings.global['mm-missed-maintenance-malus'].value / 100) *
                    const.baseAgeing
            const.repairAgeMalus = const.baseAgeing -- + const.maxMissedMaintenanceMalus * 0,5
...
script.on_event( defines.events.on_tick, function( event )
    ...
    if event.tick % (60) == 0 then
        ...
        for _, player in pairs( game.players ) do
            if global.userInterface[player.index] and
                global.userInterface[player.index].updateRequest and
                global.userInterface[player.index].root ~= nil then
    ...
    if global.eventScheduleMRO and global.eventScheduleMRO[event.tick] ~= nil then
        for _, element in ipairs( global.eventScheduleMRO[event.tick] ) do
            ...
            local entityData = global.monitoredEntities[entityID] or
                                   global.maintenanceUnits[entityID]
            ...
            if element.type == 'machine' and entityData ~= nil and
                entityData.entity and entityData.entity.valid then
                local realEntity = entityData.entity
                local entityName = entityData.surrogatedEntityName or
                                       realEntity.name
                ...
                if entityData.bonusCyclesRemaining ~= nil and
                    entityData.bonusCyclesRemaining > 0 then
                    ...
                    local entityProperties =
                        global.entitiesWithMROenabled[entityName]
                    if reduceCounter then
                        entityData.bonusCyclesRemaining =
                            entityData.bonusCyclesRemaining - 1
                        local newAgeing =
                            global.monitoredEntities[entityID].ageing +
                                entityProperties.timeFactor * const.baseAgeing
                        global.monitoredEntities[entityID].ageing = newAgeing
                    end
                    local nextTick = rand( mfloor( const.MTBM / 2 ),
                                           mfloor( const.MTBM * 1, 5 ) )
                    local eventTick = game.tick +
                                          mfloor(
                                              entityProperties.timeFactor *
                                                  nextTick )
                ...
                if status == 'operating' then
                    if not const.lowTechModifierEnabled or rand() <=
                        global.lowTechModifier[realEntity.force.index].factor then
                        ...
                        if (entityData.producedItems ~= nil or
                            const.alternativeActivityTracking[realEntity.type]) then
                        ...
                        local eventTick = game.tick +
                                              mfloor(
                                                  entityProperties.timeFactor *
                                                      nextTick )
                        global.monitoredEntities[entityID].nextEventTick =
                            eventTick
                ...
                elseif status == 'operating' then
                    if rand( 0, const.maxAge ) < entityAge *
                        const.repairProbabilityModifier then
                        -- game.print("Objekt-ID: "..entityID.." / Alter: "..element.age.." / Status: "..entityData.status)
                ...
                elseif status == 'replacement-required' then
                    local newEntityID =
                        checkForcedReplacementSuccess( entityID )
                    if tonumber( newEntityID ) ~= nil then
                        entityID = newEntityID
                        entityData = global.monitoredEntities[newEntityID]
                        realEntity = entityData.entity
                ...
                local forceSpecificControlSettings =
                    global.maintenanceControl[realEntity.force.index].byEntity[entityName]
                        .replacement
                if status == 'operating' and forceSpecificControlSettings.start >
                    0 and entityAge >= forceSpecificControlSettings.start then
            ...
            elseif element.type == 'maintenance-unit' and entityData ~= nil and
                entityData.entity and entityData.entity.valid then
